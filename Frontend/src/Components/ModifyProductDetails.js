...
const ModifyProductDetails = () => {
    ...
    const handleDelete = (productId) => {
        ...
        axios
            .delete(`${domain}/delete-product/${productId}`)
            .then((response) => {
    ...
    const handleSave = (productId) => {
        if (editingProduct && editingProduct._id === productId) {
            axios
                .patch(
                    `${domain}0/product-details-update/${productId}`,
                    editingProduct
                )
                .then(() => {
                    setProducts(
                        products.map((product) =>
                            product._id === productId ? editingProduct : product
                        )
                    );
